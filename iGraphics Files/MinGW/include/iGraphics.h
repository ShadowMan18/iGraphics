...
int iAnimDelays[10];
int iAnimPause[10];
int soundcount = 0, max_sound = 50;
enum playType1 {SYNC, ASYNC};
enum playType2 {ONCE, LOOP};
...
void playsound(char filename[], playType1 type1, playType2 type2)
{
    int play_type1, play_type2;
    if (type1 == SYNC)
    {
        play_type1 = SND_SYNC;
    }
    ...
    {
        play_type1 = SND_ASYNC;
    }
    LPWSTR name = (LPWSTR)filename;
    if (type2 == LOOP)
    {
        PlaySound(filename, NULL, play_type1 | SND_LOOP);
    }
    else
    {
        PlaySound(filename, NULL, play_type1);
    }
}
